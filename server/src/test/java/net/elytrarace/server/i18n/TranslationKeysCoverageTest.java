package net.elytrarace.server.i18n;
...
class TranslationKeysCoverageTest {
    ...
    static Stream<String[]> allServerKeys() throws IOException {
        Properties properties = new Properties();
        try (InputStream stream = TranslationKeysCoverageTest.class
                .getClassLoader()
                .getResourceAsStream("elytrarace_en_US.properties")) {
            assertThat(stream).as("elytrarace_en_US.properties must exist on the classpath").isNotNull();
    ...
    @MethodSource("allServerKeys")
    @DisplayName("Server translation values must not contain MessageFormat placeholders")
    void serverKey_usesArgSyntax_notMessageFormat(String key, String value) {
        assertThat(value)
    ...
    @MethodSource("allServerKeys")
    @DisplayName("Server translation values must not be blank")
    void serverKey_hasNonBlankValue(String key, String value) {
        assertThat(value)
    ...
    @MethodSource("allServerKeys")
    @DisplayName("Server translation arg tags use valid <arg:N> syntax")
    void serverKey_argTagsSyntaxIsValid(String key, String value) {
