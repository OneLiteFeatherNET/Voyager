...
import net.elytrarace.model.config.VoyagerConfiguration
...
class ConfigService(private val voyager: Voyager) {
    ...
    val lobbyWorld: LobbyConfig?
        get() = readLobbyWorld(voyager)

    private fun readLobbyWorld(voyager: Voyager): LobbyConfig? {
        ...
        return Json.decodeFromString<LobbyConfig?>(jsonFile.readText())?.copy(world = world)
    }
